...
* Enable across-the-years searching
    ...
    * Possible solution: Try going deeper to thread levels to get complete date format for `get_top_date()` and `get_update_date()` in ***ptt.py***. Or, for a less convenient solution, make **year** a user-input variable
